...
scrolltask(fwd := true) {
    ...
    ; get taskbar object names incl parents and children
    strings := JEE_ExpGetTaskbarItems()

    ; sift for existing handles
    loop parse, strings, `n, `r
        if (hwnd := winexist(a_loopfield))
            tasklist.push(hwnd)
	Loop % tasklist.Length()
		MsgBox % tasklist[A_Index]
    ...
    winactivate % fwd   ; jump +/-
        ? "ahk_id" . tasklist[active + 1]
        : "ahk_id" . tasklist[active - 1]
}
Return
;==================================================
...
;tested on Windows 7
JEE_ExpGetTaskbarItems(vSep="`n") ; https://www.autohotkey.com/boards/viewtopic.php?p=126047#p126047
{

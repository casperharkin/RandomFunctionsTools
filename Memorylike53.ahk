...
Loop
{
    ...
    if (myAOBaddressdec > 0) ;AOB found, stop scanning and convert it from decimal to hexadecimal
    {
        ...
        Clipboard := myAOBaddresshex
		msgbox found at %myAOBaddresshex%

		value := ReadMemoryTxt(myAOBaddresshex,"ahk_exe NOTEPAD.EXE")
		msgbox, % value 
	
		WriteMemoryT("ahk_exe NOTEPAD.EXE","22222",myAOBaddresshex)
...
;	msgbox, % value 
;
;	WriteMemoryT("ahk_exe NOTEPAD.EXE","0x54657374",myAOBaddresshex)
;		
;	value := ReadMemoryTxt(myAOBaddresshex,"ahk_exe NOTEPAD.EXE")
;	msgbox, % value 

Return
WriteMemory(PROGRAM, WVALUE, MADDRESS, Offset_1=0, Offset_2=0)
{
    winget, pid, PID, %PROGRAM%
    FADDRESS := ReadMemory(ReadMemory(ReadMemory(getProcessBaseAddress(PROGRAM) + MADDRESS, PROGRAM) + Offset_1, PROGRAM) + Offset_2, PROGRAM)
    MsgBox %FADDRESS%
    ProcessHandle := DllCall("OpenProcess", "int", 2035711, "char", 0, "UInt", PID, "UInt")
    DllCall("WriteProcessMemory", "UInt", ProcessHandle, "UInt", FADDRESS, "Uint*", WVALUE, "Uint", 4, "Uint *", 0)
...
getProcessBaseAddress(WindowTitle, Hex=1, MatchMode=2)    ;WindowTitle can be anything ahk_exe ahk_class etc
{
    ...
    if Hex
        return dectohex(DllCall("GetWindowLong", "Uint", h, "Uint", -6))
    else return DllCall("GetWindowLong", "Uint", h, "Uint", -6)
}

WriteMemoryT(PROGRAM, WVALUE, WriteAddress)
{
    winget, pid, PID, %PROGRAM%
   	VarSetCapacity(Buffer, 4, 0)
    NumPut(WVALUE, Buffer, 0, "Int") ;ensure that the number we are passing is in binary format (this shouldnt really be required though)

    ProcessHandle := DllCall("OpenProcess", "int", 2035711, "Int", 0, "UInt", PID)
    writeResult := DllCall("WriteProcessMemory"
    									, "UInt", ProcessHandle 
                                        , "UInt", WriteAddress
										, "UInt", &Buffer ; passing the address of the binary buffer
										, "UInt", 4
										, "UInt*", sizeWritten)
    DllCall("CloseHandle", "int", ProcessHandle)
    msgbox % "ProcessHandle: " ProcessHandle
    	. "`nWriteProcessMemory " (writeResult ? "Success" : "Failure")
    	. "`nBbytes Written: " sizeWritten
...
VarSetCapacity(MVALUE,32,0)
ProcessHandle := DllCall("OpenProcess", "Int", 24, "Char", 0, "UInt", pid, "UInt")
DllCall("ReadProcessMemory","UInt",ProcessHandle,"UInt",MADDRESS,"Str",MVALUE,"UInt",32,"UInt *",0)

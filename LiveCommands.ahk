...
{
Global
Gui LiveCommands: +AlwaysOnTop -Caption +ToolWindow +E0x08000000
Gui LiveCommands: Color, 0173C7
Gui LiveCommands: Font, CBlack Bold s9 Segoe UI
Gui LiveCommands: Add, edit, vEditControlMain HwndhEditControlMain x11 y7 w418 h23 +0x200 +Center +BackgroundTrans,1+1
Gui LiveCommands: Show, w437 h37 Center , Live Commands

KeyWait,Enter,D
Gui LiveCommands:  Submit, Nohide

Result := Eval(EditControlMain)
Gui LiveCommands: Destroy
MsgBox % StrJoin(Result, "`n")
}
...
NewWord := SubStr(TempList,Position,InStr(TempList,"`n",0,Position) - Position)
    ...
    Send, % "{BS " . StrLen(CurrentWord) . "}" ;clear the typed word
 	Clipboard := % HelpFile.GetSyntax(NewWord)
   Send ^v

;msgbox % HelpFile.GetSyntax(NewWord)
